package com.ruby.java.agrt.Ch09;
...
class Tree5 {
	...
	boolean insert(int x) {// binary search tree를 만드는 입력 : left subtree < 노드 x < right subtree
		// inorder traversal시에 정렬된 결과가 나와야 한다 -- 미완
		TreeNode5 p = root;
		TreeNode5 q = null;
...
//		}
		...
		while (p != null) {
			...
			if (x < p.data) {
				p = p.LeftChild; // 노드 n 기준 왼쪽 서브트리 노드의 모든 키 값은 노드 n 의 키 값 보다 작음
			} else {
				p = p.RightChild; // 오른쪽 서브트리 노드의 키값은 노드 n 의 키값 보다 큼
			}
		}
			p = new TreeNode5();
			p.data = x;
	...
	boolean delete(int num) {// binary search tree에서 임의 값을 갖는 노드를 찾아 삭제한다. -- 미완
	...
	boolean search(int num) {// num 값을 binary search tree에서 검색 -- 구현 완료
		TreeNode5 p = root;
		if(root == null) {
			return false;
		}
